...
\newcommand{\rev}[2][inline]{\color{blue}#2\color{black}}
...
% Abstract
\begin{abstract}
\rev{In the past decades, } one of the most common forms of addressing reproducibility in 
scientific workflow-based computational science has consisted \rev{of }
...
the experiments are conducted. In this work, we propose a novel approach \rev{based
on semantic vocabularies that describes } the execution environment of scientific workflows, 
so as to conserve \rev{it}. We define a process for documenting the workflow 
application and its related management system, as well as their dependencies. 
Then we apply this approach over three different real workflow applications \rev{running in } three 
\rev{distinct } scenarios, using public, private, and local Cloud platforms. In particular, we study 
