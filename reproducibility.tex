...
\note{Rafael, check new abstract. I have modified the abstract, as it was the same as in REPPAR and XSEDE papers. The original one is commented}

Reproducibility in computational science has been addressed in last decades by tracking the provenance of the results produced and published, leaving out of the scope the execution environment of performed experiment. This approach allows inspecting and improves understanding of both intermediate and final results, but it does not provide any means for sharing the valuable knowledge about the experimental equipment of a computational experiment. In this work, we propose a new approach for describing the execution environment of scientific workflows, so as to conserve them, using semantic vocabularies. We define a process for  documenting the workflow application and its related management system, as well as their dependencies. We apply this approach over three different real workflow applications on three different scenarios, using public, private and local Cloud platforms. Results show that our approach can reproduce an equivalent execution environment of a predefined virtual machine image on all computing platforms.
