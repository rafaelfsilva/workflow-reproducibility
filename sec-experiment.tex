...
(lines 23--25). To enact Vagrant support, we created an intermediate phase (\emph{Abstract 
Deployment Plan}, step 9 in Figure~\ref{fig:wicusflow}), which defines the steps and 
scripts to be executed, along with their configuration parameters. ISA then concretizes 
this plan into a PRECIP or Vagrant script depending on the resultant targeted provider 
(line 27).
...
In this experiment, we execute ISA over the annotated data specifying the three different providers we work with. As result we obtain an abstract plan which, based on the \texttt{WICUS Stack}\footnote{http://purl.org/net/wicus-stack} domain ontology, defines the stacks must be deployed. As depicted in figure~\ref{fig:stack-rel}, a \texttt{Software Stack} contains one or more \texttt{Software Components}. Each of them has an associated \texttt{Deployment Plan}, composed by one or more \texttt{Deployment Steps}.

\begin{figure}[!htt]
	\centering
	\includegraphics[width=0.9\linewidth]{figures/stack-rel}
	\caption{WICUS software stack relation diagram overview.}
	\label{fig:stack-rel}
\end{figure}

Listing \ref{lst:plan-soykb} describes the abstract plan of the SoyKB workflow. The first section of the plan (lines 1-27) describe the deployment of the \texttt{Pegasus WMS} and its related dependencies, and is common to the three workflows covered in this work. The following lines depict how the SoyKB software is deployed. The \texttt{SOFTWARE.TAR.GZ} stack, which a dependency of all the SoyKB wrappers, is deployed before them (lines 28-30). The last section of the plan (lines 31-46) describes how the four SoyKB wrappers are deployed. Each one of them requires two deployment steps, one for copying the binary itself and another for granting proper execution permission. 
