...
\note{Idafen: talk about the WMS requirements.}

As depicted on figure~\ref{fig:annotations} the three workflows included on this
work are linked to the WMS requirements, meaning that for executing them
\texttt{Java}, \texttt{wget}, \texttt{HTCondor}, and \texttt{Pegasus} must be
installed on the target computational resource.
...
\note{Idafen, explain why we have images with larger disks, justifying it by saying that we try to reduce resource consumption}

\begin{table}[h]
\begin{tabular}{l|lllll}
\multicolumn{1}{c|}{} 
Img. App. & AWS1 & AWS2 & FG1 & FG2 & Vagrant \\ \hline
RAM (GB) & 7 &  7 & 8 & 8 &  4 \\ \hline
