...
%-- modified based on oscar's feedback
...
resources at distributed locations\rev{ It is based on OpenStack\footnote{https://www.openstack.org/}, 
as our system would be able to work with any OpenStack cloud}. Amazon Web Services EC2 is a public infrastructure provider, and the \emph{de facto} standard for IaaS Cloud platforms. 
...
Linked Data~\cite{XXX} principles, as it uses HTTP URIs, exposing them using RDF standards. }

\todo{add linked data citation}
...
Catalog (step 7 in Figure~\ref{fig:wicusflow}). \rev{According to the SVA model, an 
...
(software and hardware) specified for the SVA.  Thus, we defined  an Image Appliance that groups 
all Image Appliances into the \texttt{CentOS 6 SVA}. We will have five different Image Appliances 
...
\rev{\subsection{Reproduction Results}}
...
\rev{
The obtained results show scientific experiments in form of computational workflows can be reproduced based on semantic annotations using a proper vocabulary. In this work we introduce a set of tools for generating those annotations, which ease the effort of documenting the execution environment. However, those tools do not provide a fully automated process and some manual interactions is still required. Two of the entry files of the reproducibility process (Figure \ref{fig:wicusflow}), the \emph{WMS Annotations} and the \emph{SVA Catalog}, are generated manually. These files are reused for the three experiments exposed in this section, and would be also used for new Pegasus workflows. When reproducing a workflow belonging to a different WMS, a new set of annotations about it would be necessary, whereas the \emph{SVA Catalog} could be reused. This catalog is extended as new computational resources are added to the system.

Most of the software involved on the execution of the three workflows studied in this work was, and still is, available online either through distribution packages repositories or hosted online by different institutions. Other software applications, such as the ones designed for the workflow, have been hosted in a public repository for this work. We decided to upload them to GItHub\footnote{https://github.com/} to ensure that they could be retrieved remotely. 
}
