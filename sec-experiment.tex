...
The goal of this experiment is to reproduce original workflow executions \rev{on } the three different 
...
network. All the annotations generated in this work are available \rev{through } the 
...
represent its dependencies. Pegasus relies on \texttt{HTCondor} as \rev{the } task manager, and 
...
for each transformation (Figure~\ref{fig:annotations-genome}). \rev{We then annotate } 
...
\rev{in } a software bundle that is deployed on the computational nodes. Hence, a 
...
Cloud resources due to the cost \rev{of } storing/transferring VM images. Since Vagrant
...
is defined by the set of software elements associated \rev{with } it, while Image Appliances 
...
on the \rev{relationship } between the requirements and the component dependencies. This 
...
execution platform. Figure~\ref{fig:stack-rel} shows the \rev{relationships } between the different
elements that compose the \texttt{Stack} domain ontology. A \texttt{Software Stack} may
be composed \rev{of } one or more \texttt{Software Components}. Each of them has an associated 
\texttt{Deployment Plan} according to the target execution platform, which is composed \rev{of } 
...
platforms. That is, a total of 9 different executions, as each workflow is executed \rev{on } 
