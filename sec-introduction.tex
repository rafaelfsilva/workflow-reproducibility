...
We conduct a practical experimentation process in which we describe a set workflows and their environments using a set of semantic models. Then, we use an experiment management tool to reproduce a 
workflow execution in different Cloud platforms. \rev{In this work we study three real and representative production workflows, which work under a single node configuration, which have been disseminated in several scientific publications. Many scientific workflows allow their execution infrastructure to be escalated, depending on the processing needs of the application. Currently, we have restricted our approach to single computer experiments, which cover a wide range of computational experiments.}
